...
function Summary:Create(parentFrame)
    ...
    function f:LayoutCards()
        ...
        if self.graphStyleDropdown and self.cards and self.cards[1] then
            ...
            self.graphStyleDropdown:SetPoint("TOPLEFT", self.cards[1], "TOPLEFT", -10, 30)
        end
...
function Summary:Refresh()
    ...
    for i, bracket in ipairs(BRACKETS) do
        ...
        local last25Text = ""
        if last25Count > 0 then last25Text = string.format("%d games played  %+d CR", last25Count, last25Delta) end
